    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel">场景文件</h4>
            </div>
            <textarea id="sceneSource" style="width:100%;height:600px;border:none;resize:none;">在这里添加一些文本</textarea>
...
function getSceneJsonString(){
    ...
    return `return {
        DirectLight:{
            enable:${vox.enableDirectLight},
            shadow:${light1.castShadow},
            ${color(light1.color)},
            ${position(light1.position)}
        },
        SpotLight:{
            enable:${vox.enableSpotLight},
            shadow:${light2.castShadow},
            ${color(light2.color)},
            ${position(light2.position)}
        },
        HemiLight:{
            enable:${vox.enableHemiLight},
            ${color(hemiLight.color,"skyColor")},
            ${color(hemiLight.groundColor,"groundColor")},
            ${position(hemiLight.position)}
        },
        Camera:{
            ${position(camera.position)},
            ${position(camera.rotation,'euler')}
        }
    };`;
}
 ...
 * 应用场景信息
 */
function setSceneJsonString(jsonStr){
}
...
function initTest(){
    vox = {
        ...
        "刷新模型列表" : function(){
            fetchJson('resources',function(json){
                if(gui_modules)gui_modules.remove();
                let voxs = json.files.filter(item=>item.match(/.*\.vox$/));
                gui_modules = gui_vox.add(vox,'model',voxs);
            });            
        },
        "添加" : function(){
        ...
        open : function(){
            fetch(`scene/${this.file}`).then(function(response){
